...
export default function evaluate(input: Input, output: Output): {score: number, nCarsFinished: number} {
    var score = 0
    var nCarsFinished = 0
    ...
    for(let t=0; t<=simulationTime; t++){
        const alreadyPassedIntersections: number[] = [] //To allow only one car per intersection at each step
